...
def collect_presentations(*, ready_only: bool, exclude_blank_id: bool):
    ...
    queryset = MergedPresentation.objects.order_by("presentation_id", "name", "id")
    if ready_only:
...
def build_duplicate_groups(presentations) -> Dict[str, List[object]]:
    ...
    for presentation in presentations:
        groups[presentation.presentation_id or ""].append(presentation)

    return {
        presentation_id: group
        for presentation_id, group in groups.items()
        if presentation_id and len(group) > 1
    }
...
def _print_duplicate_group(group_index: int, total_groups: int, presentation_id: str, presentations):
    print("\n" + "=" * 80)
    print(f"[{group_index}/{total_groups}] Duplicate presentation_id: {presentation_id}")
...
def resolve_duplicate_groups(duplicate_groups):
    ...
    for group_index, (presentation_id, presentations) in enumerate(sorted_groups, start=1):
        _print_duplicate_group(group_index, len(sorted_groups), presentation_id, presentations)
        ...
        while True:
            ...
            print(
                ...
                f"{len(delete_presentations)} other row(s) for presentation_id={presentation_id}."
            )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Interactively resolve duplicate MergedPresentation rows by presentation_id."
        )
    )
    parser.add_argument(
        "--ready-only",
    ...
    if not duplicate_groups:
        print("No duplicate presentation_ids found.")
        return 0

    print(f"Found {len(duplicate_groups)} duplicate presentation_id group(s).")
